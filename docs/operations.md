...
`notion-dev-refactor-guard` is centered on developer tools work in Rust. The technical target is: Build a Rust toolkit that studies refactor behavior through layout fixtures, with stable geometry snapshots and bounded memory input sets.
...
The score is deterministic. The current threshold is 166. Risk has a penalty of 5, latency has a penalty of 2, and weight contributes a bonus of 6.
